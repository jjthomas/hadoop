...
public class TestDecommissioningStatus {
  ...
  private FSDataOutputStream writeIncompleteFile(FileSystem fileSys, Path name,
    ...
    // need to make sure the block size is a multiple of the buffer size so
    // that we actually write out both file blocks during the incomplete write
    FSOutputSummer.setNumChunksToBuffer(2);
